...
| `api-sync.py` | copies the code repositories' rendered `docs/api/*.json` into `api/` and stamps `api/index.json` with the commits; `--check` refuses a stale or malformed snapshot |
| `api/` | the API as data, published: `mcp-tools.json`, `daemon-rpc.json`, `note.schema.json`, `openapi.json`, `index.json` — generated, never edited |
| `llms.txt` | the front door for a program: the product in one paragraph, the four documents, the rules agents get wrong from tool names alone, the pages — generated by `build.py` |
